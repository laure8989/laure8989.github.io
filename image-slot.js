...
(() => {
  // A page-scoped filename (derived from location.pathname) seemed like
  // the right fix for the shared-file size cap, but the host's write
  // allowlist only permits the literal '.image-slots.state.json' name —
  // any other basename gets rejected ("path not allowed"), even though it
  // still ends in '.state.json'. Back to one shared file; size is managed
  // via MAX_DIM/quality below instead.
  const STATE_FILE = '.image-slots.state.json';

  // Persistent debug log: survives a reload (unlike console output), so
  // instrumenting a race that involves a reload/remount stays inspectable
  // after the fact via localStorage.getItem('__imageSlotLog').
  const LOG_KEY = '__imageSlotLog';
  function dlog(...args) {
    const line = new Date().toISOString().slice(11, 23) + ' ' + args.map((a) => {
      try { return typeof a === 'string' ? a : JSON.stringify(a); } catch { return String(a); }
    }).join(' ');
    console.log('[image-slot]', line);
    try {
      const buf = JSON.parse(localStorage.getItem(LOG_KEY) || '[]');
      buf.push(line);
      while (buf.length > 300) buf.shift();
  ...
  // sidecar enormous. A 900px WebP at q=0.78 is ~70-140KB. The sidecar is
  // shared across every page in the project (host write allowlist permits
  // only this one literal filename), so this stays conservative rather
  // than the 1600/0.85 a per-page file would have allowed.
  const MAX_DIM = 900;
  ...
  function flushNow() {
    ...
    if (!w) { dlog('flushNow: writeFile unavailable — cannot persist'); return; }
    try {
      Promise.resolve(w(STATE_FILE, JSON.stringify(slots)))
        .then(() => dlog('flushNow: wrote', Object.keys(slots).length, 'slot(s)'))
        .catch((e) => dlog('flushNow: write FAILED', e && (e.message || String(e))));
    } catch (e) { dlog('flushNow: write threw', e && (e.message || String(e))); }
  }
  function save() {
    if (saving) { saveDirty = true; dlog('save: already in flight — marked dirty for re-fire'); return; }
    const w = window.omelette && window.omelette.writeFile;
    if (!w) { dlog('save: writeFile unavailable — cannot persist'); return; }
    saving = true;
    dlog('save: write STARTED —', Object.keys(slots).length, 'slot(s):', Object.keys(slots).join(','));
    Promise.resolve(w(STATE_FILE, JSON.stringify(slots)))
      .then(() => dlog('save: write COMPLETED'))
      .catch((e) => dlog('save: write FAILED', e && (e.message || String(e))))
  ...
  function setSlot(id, val) {
    if (!id) { dlog('setSlot: no id — cannot persist'); return; }
    dlog('setSlot: id resolved =', id);
    if (val) { slots[id] = val; tombstones.delete(id); }
    else { delete slots[id]; if (!loaded) tombstones.add(id); }
    dlog('setSlot: in-memory state updated for', id, '— total slots now', Object.keys(slots).length);
    subs.forEach((fn) => fn());
    if (loaded) { dlog('setSlot: flush scheduled for', id, '(hydrated)'); save(); }
    else { dlog('setSlot: flush deferred for', id, '— awaiting sidecar hydration'); load().then(save); }
  }
  // Debug hook for tracing persistence issues from outside the closure.
  window.__imageSlotDebug = { getSlot, setSlot, save, flushNow, get slots() { return slots; }, get loaded() { return loaded; } };

  // Global unload safety net: a plain drop's save() is async and had no
  // flush-on-unload guard (only reframe's pagehide handler did) — a reload
  // right after dropping could abandon the in-flight write. Every setSlot
  // (drop OR reframe) now lands here regardless of which UI path triggered it.
  ...
  async function toDataUrl(file, targetW, maxDim, quality) {
    const bitmap = await createImageBitmap(file);
    try {
      const cap = Math.min(maxDim, Math.max(1, Math.round(targetW * 2)) || maxDim);
  ...
  // ── Lightbox ─────────────────────────────────────────────────────────
  // Singleton full-size preview overlay, shared by every slot on the page.
  // Lives in the light DOM (document.body) so position:fixed always covers
  // the full viewport regardless of any shadow root / overflow ancestor.
  let lightboxEl = null;
  function ensureLightbox() {
    ...
    el.style.cssText = 'position:fixed;inset:0;z-index:2147483000;display:none;' +
      'align-items:center;justify-content:center;background:rgba(10,10,8,.92);' +
      'padding:5vw;box-sizing:border-box;cursor:zoom-out';
    ...
    closeBtn.style.cssText = 'position:fixed;top:20px;right:24px;appearance:none;border:0;' +
      'border-radius:8px;padding:10px 14px;background:rgba(255,255,255,.12);color:#fff;' +
      'font:16px/1 system-ui,-apple-system,sans-serif;cursor:pointer;backdrop-filter:blur(6px)';
    closeBtn.addEventListener('click', (e) => { e.stopPropagation(); hideLightbox(); });
    img.addEventListener('click', (e) => e.stopPropagation());
    el.addEventListener('click', hideLightbox);
    el.appendChild(img); el.appendChild(closeBtn);
    document.body.appendChild(el);
    lightboxEl = el;
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape' && el.style.display !== 'none') hideLightbox();
    });
    return el;
  }
  ...
  const stylesheet =
    ...
    ':host([data-filled]) .frame{cursor:zoom-in}' +
    ':host([data-no-lightbox][data-filled]) .frame{cursor:inherit}' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.addEventListener('click', (e) => {
        ...
        if (!act) {
          // Click the image itself (not the empty state, not mid-reframe,
          // not the credit chip which already stops propagation) to view
          // it full-size. Works in both the editor preview and a plain
          // read-only page — it's a viewing affordance, not an edit one.
          if (this.hasAttribute('data-filled') && !this.hasAttribute('data-reframe') &&
              !this.hasAttribute('data-no-lightbox') &&
              e.target.closest && e.target.closest('.frame')) {
            showLightbox(this._img.src);
          }
          return;
        }
    ...
    async _ingest(file) {
      this._setError(null);
      dlog('file received for', this.id || '(no id)', '—', file && file.name, file && file.type, (file && file.size) + 'b');
      if (!file || ACCEPT.indexOf(file.type) < 0) {
        this._setError('Drop a PNG, JPEG, WebP, or AVIF image.');
        dlog('rejected file type for', this.id, file && file.type);
        return;
      }
      ...
      try {
        ...
        // A slot can opt into a bigger/sharper encode via data-max-dim /
        // data-quality (e.g. a full-bleed hero) — everything else stays on
        // the conservative default so the shared sidecar doesn't refill
        // the write-size cap.
        const maxDim = parseInt(this.getAttribute('data-max-dim'), 10) || MAX_DIM;
        const quality = parseFloat(this.getAttribute('data-quality')) || 0.78;
        const url = await toDataUrl(file, w, maxDim, quality);
